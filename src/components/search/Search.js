    ...
    return (
        <div className="search-container mb-10 lg:mb-16">
            ...
            lg:flex-row lg:justify-between">
                <div className='input mt-4 lg:mt-12  flex flex-col items-center relative w-[100%] lg:w-[40%] '><MagnifyingGlassIcon className='w-5 text-LtModTxtmdInp opacity-50 absolute left-8 top-[30%] ' /><input className='shadow-lg focus:shadow-xl w-[100%] text-LtModTxtmdInp rounded-md placeholder-LtModTxtmdInp pl-16 placeholder:text-sm placeholder:opacity-50 py-3 px-4 focus:outline-none' type="text" value={name} onChange={OnChangeurl} placeholder='Search for a country...' /></div>

                <div onClick={ToggleReion} className="filter px-5 w-[180px] shadow-lg py-3 rounded-md flex items-center justify-between relative cursor-pointer">
                    <p className='text-sm'>{regionFilter}</p> <span className=' caret'>{toggleRegion ? <ChevronDownIcon className='h-4 font-800 text-LtModTxtmdInp' /> : <ChevronUpIcon className='h-4 font-800 text-LtModTxtmdInp' />}</span>
